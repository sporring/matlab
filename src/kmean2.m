...
%         K - is the initial set of clusters
%
...
%       algorithm is the simplest but also slowest possible.  The squared
%       distance is sought to be minimized from the modified image to the
%       original using only the values from K. Thereafter K is modified as
%       the center of the belonging kluster.  Kluster-centers with no
%       members are removed.
%
%       To reduce the number of intensities to 4 (investigating only one
%       starting point) in matrix I use,
%         [K, J, E] = kmean(I, 255*rand(1,4));
%
